  ...
  elseif P1=="?report" or P1=="?log" then
    ...
    elseif not (QDKP2_IsInGuild(P2) or P2 == "RAID") then
      ...
      elseif P3=="all" then
        ...
        return {'QDKP2 - 错误的报告类型。 用法: "?report <name> all/current"'}
      end
...
--[[
  ...
  elseif (P1=="?classdkp" or P1=="?dkpclass") then
    ...
    if not QDKP2_COD then return {"QDKP2 - 这项功能未启用。"}; end
    local arg=P2 or QDKP2class[sender]
    if not arg or arg=="--" then return {"QDKP2 - 你必须输入想要查询的职业。"}; end
    local origArg=arg
    local class
    ...
    if table.getn(list)==0 then return {"QDKP2 - 公会中没有该职业。"}; end
    QDKP2_netSort(list)
  ...
  elseif (P1=="?rankdkp" or P1=="?dkprank") then
    ...
    if not QDKP2_KOD then return {"QDKP2 - 这项功能未启用。"}; end
    ...
    if not rank then return {"QDKP2 - 你必须输入想要查询的会阶。"}; end
    ...
    if table.getn(list)==0 then return {"QDKP2 - 该会阶下没有公会成员"}; end
    QDKP2_netSort(list)
    for i=1,table.getn(list) do
      if i > QDKP2_LOD_MAXLEN then break; end
      local name=list[i]
  ...
  elseif P1=="?help" or P1=="?commands" or P1=="?command"  or P1=="?keyword"  or P1=="?keywords" then
    ...
    local output={  "QDKP2 - 密语问询系统关键词列表:"}
