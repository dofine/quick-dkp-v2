function QDKP2GUI_MiniBtn_Click(self, ...)
  ...
  function QDKP2GUI_MiniBtn_LabelOn(self,...)
    ...
    if QDKP2_OfficerMode() then
      GameTooltip:AddLine("官员模式",0.3,1,0.3)
    else
      GameTooltip:AddLine("只读模式",1,1,1)
    end
    local M1,M2,M3 = QDKP2_GetPermissions()
    GameTooltip:AddLine("DKP 官员权限:")
    ...
    GameTooltip:AddLine("左键单击: 显示/隐藏 QDKP",.8,.8,.8,1)
    GameTooltip:AddLine("SHIFT+CLICK: Drag self button",.8,.8,.8,1)
	GameTooltip:AddLine("右键单击: 打开 Quick DKP 配置界面",.8,.8,.8,1)
    GameTooltip:Show()
  end
